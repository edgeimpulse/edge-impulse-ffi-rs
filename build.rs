...
fn extract_and_write_model_metadata() {
    ...
    let header = fs::read_to_string(header_path)
        .expect("Failed to read model_metadata.h");
    ...
    for (name, val) in &raw_defs {
        ...
        if val == "uint8_t" || val == "bool" || val == "size_t" { continue; }
...
", name, num));
                ...
                out.push_str(&format!("// Could not resolve EI_CLASSIFIER_SENSOR as i32: {}\n", resolved));
            }
            continue;
        }
        // String constants
...
", name, resolved));
        ...
        if name == "EI_CLASSIFIER_SLICE_SIZE" {
            ...
            let raw_sample_count = emitted.get("EI_CLASSIFIER_RAW_SAMPLE_COUNT").and_then(|v| v.parse::<usize>().ok()).unwrap_or(0);
            let slices_per_window = emitted.get("EI_CLASSIFIER_SLICES_PER_MODEL_WINDOW").and_then(|v| v.parse::<usize>().ok()).unwrap_or(1);
            let value = raw_sample_count / slices_per_window;
            out.push_str(&format!("pub const EI_CLASSIFIER_SLICE_SIZE: usize = {};
", value));
        ...
        if name == "EI_CLASSIFIER_RESIZE_MODE" {
            ...
            out.push_str("pub const EI_CLASSIFIER_RESIZE_MODE: usize = EI_CLASSIFIER_RESIZE_SQUASH;\n");
...
fn main() {
    ...
    println!("cargo:rerun-if-changed={}/edge_impulse_wrapper.h", model_dir);
